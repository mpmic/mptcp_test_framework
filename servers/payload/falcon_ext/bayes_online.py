...
ConstHazardNumba = numba.jitclass([("time_scale", numba.float64)])(ConstHazard)
...
@numba.jitclass(
    [
        ("_alpha0", numba.float64),
        ("_beta0", numba.float64),
        ("_kappa0", numba.float64),
        ("_mu0", numba.float64),
        ("_alpha", numba.float64[:]),
        ("_beta", numba.float64[:]),
        ("_kappa", numba.float64[:]),
        ("_mu", numba.float64[:]),
    ]
)
...
class BayesOnline:
    ...
    def __init__(
        ...
        hazard_params={"time_scale": 250.0},
        obs_params={"alpha": 0.1, "beta": 0.01, "kappa": 1.0, "mu": 0.0},
        engine="numba",
    ):
        """Parameters
        ----------
    ...
    def update(self, x):
        ...
        if self._use_numba:
            new_p = segmentation_step_numba(x, old_p, self.hazard, self.obs_likelihood)
        else:
            new_p = segmentation_step(x, old_p, self.hazard, self.obs_likelihood)
