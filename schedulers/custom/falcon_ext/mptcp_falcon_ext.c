...
static int mptcp_dont_reinject_skb(const struct tcp_sock *tp, const struct sk_buff *skb)
{
 ...
 /* Try to use the subflow chosen in userspace with setting msp->num_segments=1*/
 //if dedicated subflow isnt available as bestsk any other available subflow can be chosen as backupsk
 //in this case the pathmask is also reset (not sure what this does but included in all schedulers in this function)
				   ...
				   bool zero_wnd_test)
{
	...
	if (meta_sk->sk_shutdown & RCV_SHUTDOWN &&
		...
		mptcp_for_each_sub(mpcb, mptcp) {
			...
			if (tcp_sk(sk)->mptcp->path_index == mpcb->dfin_path_index &&
			    mptcp_f_is_available(sk, skb, zero_wnd_test,true))
				return sk;
		}
	}
	...
	mptcp_for_each_sub(mpcb, mptcp) {
		...
		//pr_info("chosen subflow ?: %d",fsp->num_segments);


		if(fsp->num_segments==0) //only use the falcon selected subflow
			continue;

		if (!mptcp_f_is_available(sk,skb,false,false)){ //only if available
			continue;
		}

		bestsk = sk;
	}
	//always reset path mask after path chosen
	if (bestsk) {
		sk = bestsk;
		if(skb)
...
// next segment function coped from mptcp_blest
...
struct mptcp_sched_ops mptcp_sched_falcon_ext = {
...
static int __init f_ext_register(void)
{
    BUILD_BUG_ON(sizeof(struct fsched_priv) > MPTCP_SCHED_SIZE);
    BUILD_BUG_ON(sizeof(struct rbuf_opti) > MPTCP_SCHED_SIZE);

    printk(KERN_INFO " falcon_ext scheduler is loaded\n");

    if (mptcp_register_scheduler(&mptcp_sched_falcon_ext))
        return -1;

    return 0;
}

static void f_ext_unregister(void)
{
    mptcp_unregister_scheduler(&mptcp_sched_falcon_ext);
}

module_init(f_ext_register);
module_exit(f_ext_unregister);

MODULE_AUTHOR("ME");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Falcon_ext scheduler for mptcp, based on blest");
MODULE_VERSION("0.96");
