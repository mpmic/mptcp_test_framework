...
static struct mptcp_sched_ops mptcp_sched_reles_ext = {
...
static int __init reles_ext_register(void)
{
    BUILD_BUG_ON(sizeof(struct mysched_priv) > MPTCP_SCHED_SIZE);

    printk(KERN_INFO "Hello world reles_ext scheduler is loaded\n");

    if (mptcp_register_scheduler(&mptcp_sched_reles_ext))
        return -1;

    return 0;
}

static void reles_ext_unregister(void)
{
    mptcp_unregister_scheduler(&mptcp_sched_reles_ext);
}

module_init(reles_ext_register);
module_exit(reles_ext_unregister);

MODULE_AUTHOR("Gao gogo");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("RELES_ext scheduler for mptcp");
MODULE_VERSION("0.91");
